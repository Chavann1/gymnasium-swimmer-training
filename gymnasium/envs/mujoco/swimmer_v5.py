...
class SwimmerEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        action_smoothness_weight: float = 1e-3,
        stability_weight: float = 1e-4,
        action_change_weight: float = 1e-3,
        distance_weight: float = 0.5,
        # Curriculum level (0 = easiest)
        ...
        **kwargs,
        ...
        utils.EzPickle.__init__(
            ...
            xml_file,
            frame_skip,
            default_camera_config, reward, reward_info = self._get_rew(x_velocity, action, prev_action=self._prev_action)
            # update previous action for smoothness penalties next step
            self._prev_action = copy.deepcopy(action)
            ctrl_cost_weight,
            reset_noise_scale,
        ...
        self._action_smoothness_weight = action_smoothness_weight
        self._stability_weight = stability_weight
        self._action_change_weight = action_change_weight
        ...
        try:
            ...
            action_shape = (self.model.nu,) if hasattr(self.model, "nu") else (0,)
        self._prev_action = np.zeros(action_shape, dtype=np.float64)

    # Default PPO hyperparameters tuned for stability / faster convergence
    ...
    def get_default_ppo_hyperparams(self):
        """Return recommended PPO hyperparameters for training on this env."""
        return dict(self.DEFAULT_PPO_HYPERPARAMS)

    def set_curriculum_level(self, level: int):
        """Set curriculum difficulty level. Higher = harder (more noise).

        Users can call `advance_curriculum()` from training loop to increase difficulty.
        """
        self._curriculum_level = max(0, int(level))

    def advance_curriculum(self, step: int = 1):
        """Increase curriculum level by `step`."""
        self.set_curriculum_level(self._curriculum_level + step)
    ...
    def reset_model(self):
        # Apply curriculum scaling: higher levels increase reset noise (harder)
        curriculum_scale = 1.0 + 0.3 * float(self._curriculum_level)
        noise_low = -self._reset_noise_scale * curriculum_scale
        noise_high = self._reset_noise_scale * curriculum_scale
        ...
        # reset prev action to zeros so first-step penalties are not extreme
        try:
